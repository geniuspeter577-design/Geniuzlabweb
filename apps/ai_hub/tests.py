...
class ChatSendApiTests(TestCase):
    ...
    @patch("apps.ai_hub.views.stream_chat_reply")
    ...
    def test_conversation_title_set_from_first_message(self):
        ...
        with patch("apps.ai_hub.views.stream_chat_reply", return_value=iter(["ok"])):
            self.client.post(
                self.url,
...
class ChatProviderUnitTests(TestCase):
    ...
    @patch.dict("os.environ", {"OPENAI_API_KEY": "sk-test"})
    @patch("apps.ai_hub.chat_provider.requests.post")
...
class ImageGeneratorTests(TestCase):
    ...
    @patch.dict("os.environ", {"OPENAI_API_KEY": "sk-test"})
    @patch("apps.ai_hub.providers.requests.post")
...
class VideoGeneratorTests(TestCase):
    ...
    @patch("apps.ai_hub.video_providers.requests.post")
